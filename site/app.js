...
const timestampIsInput = document.querySelector("#timestamp-is");
...
const reportOutputNode = document.querySelector("#report-output");
...
async function runCalculation() {
  ...
  const timestampIs = timestampIsInput.value || "end";
  ...
  try {
    ...
    const load = readLoadProfile(loadText, timestampIs);
    if (!load.length) {
...
function readLoadProfile(text, timestampIs) {
  const { columns, rows } = parseDelimitedText(text);
  const timeColumn = pickExisting(columns, ["Messzeitpunkt", "Zeitstempel", "timestamp", "datetime", "Datum", "date"]);
  const consumptionColumn = pickExisting(columns, ["Verbrauch [kWh]", "Verbrauch[kWh]", "Verbrauch", "consumption_kwh", "kwh"]);
  const intervalColumn = pickExisting(columns, ["Messinterval", "Messintervall", "resolution", "Intervall"]);
  ...
  return rows
    .map((row) => {
      ...
      const intervalStart = timestampIs === "start"
        ? new Date(timestamp.getTime())
        : new Date(timestamp.getTime() - intervalMs);
      const intervalEnd = timestampIs === "start"
        ? new Date(timestamp.getTime() + intervalMs)
        : new Date(timestamp.getTime());
...
function renderKpis(summary) {
  const cards = [
    ...
    },
    {
      ...
      note: `${number3.format(summary.missing_price_consumption_kwh)} kWh ohne Preis`,
    },
  ];
...
function renderSummaryTable(summary) {
  const rows = [
    ...
    },
    {
      ...
      note: "Aufschlag in ct/kWh netto auf den tatsaechlichen Verbrauch.",
    },
    {
      label: "Gesamt netto",
      value: currency.format(summary.total_net_eur),
      note: "Spotmarktpreis plus Lieferanten-Offset, ohne USt.",
    },
    {
      label: "Gesamt brutto",
      value: currency.format(summary.total_gross_eur),
      note: "Spotmarktpreis plus Lieferanten-Offset und USt.",
    },
    {
...
function renderReport(summary) {
  reportOutputNode.textContent = buildReport(summary);
}
...
function drawYAxis(svg, bounds, yMax, label, color = "#455a57", side = "left") {
  ...
  svg.append(textNode(side === "left" ? 18 : bounds.width - 18, bounds.top - 4, label, {
    fill: color,
    "font-size": 12,
